"""
Inference script with logging for cartpole policy evaluation.
...
- Pole angle, pole velocity, cart position, cart velocity
- Cumulative return

Usage:
    python scripts/inverted_pendulum/inference.py \
        --num_envs 2 \
        --num_episodes 5 \
        --checkpoint_path scripts/inverted_pendulum/logs/checkpoints/cartpole_balance/best.pt
...
parser = argparse.ArgumentParser(description="Cartpole policy inference with logging")
parser.add_argument("--num_envs", type=int, default=2, help="Number of environments to spawn.")
parser.add_argument("--num_episodes", type=int, default=5, help="Number of episodes to run per env.")
parser.add_argument("--test_name", type=str, default="cartpole_test", help="Name for this test run.")
...
class TestLogger:
    """Logs per-step data for cartpole policy evaluation."""
    ...
    def log_step(
        ...
        for env_id in self.env_ids_to_log:
            ...
            # Decompose obs: [pole_angle, pole_vel, cart_pos, cart_vel]
            buf["pole_angle"].append(float(policy_obs[0]))
            buf["pole_vel"].append(float(policy_obs[1]))
            buf["cart_pos"].append(float(policy_obs[2]))
            buf["cart_vel"].append(float(policy_obs[3]))
...
def set_camera_pose(env, camera_id: int, env_id: int):
    """Set camera to look at the cartpole from the side."""
    unwrapped = env.unwrapped

    # Cart sits at height ~2.0 (from CARTPOLE_CFG default root state)
    # Point camera at the cart origin for this env
    cart_root = unwrapped.cartpole.data.default_root_state[env_id, :3].clone()
    camera_target = cart_root + unwrapped.scene.env_origins[env_id]

    # Side view: offset in +X, slightly above
    eye_camera = camera_target + torch.tensor([8.0, 0.0, 1.0], device=unwrapped.device)
...
def make_env(num_envs: int, output_type: str = "torch", enable_cameras: bool = False):
    id_name = "cartpole-swingup-v0-inference"
    gym.register(
        id=id_name,
        entry_point="scripts.inverted_pendulum.envs.cartpole_swingup_env:CartpoleSwingUpEnv",
        disable_env_checker=True,
        kwargs={
            "env_cfg_entry_point": "scripts.inverted_pendulum.envs.cartpole_swingup_env:CartpoleSwingUpEnvCfg",
        },
    )
...
def run_inference(
    ...
    # Set camera poses (one camera per logged env)
    if enable_cameras:
...
def main():
    ...
    # Load config for network architecture
    ...
    env_ids_to_log = list(range(min(args_cli.num_envs, 2)))  # log first 2 envs
